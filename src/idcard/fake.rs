...
use super::{Error, Gender, get_check_code, get_weights_sum, region, string_to_integer_array};
...
pub fn new(
    ...
    let mut rng = rng();
    let mut seq = rng.random_range(0..999);
...
pub fn rand_with(options: &FakeOptions) -> Result<String, Error> {
    ...
    let age = if max_age == min_age {
        ...
        rng.random_range(min_age..=max_age)
    };
    ...
    let gender = if let Some(value) = &options.gender {
        ...
        let flag = rng.random_range(0..10);
        if flag % 2 == 0 {
            Gender::Male
        } else {
            Gender::Female
...
mod tests {
    ...
    fn test_rand_with_options() {
        ...
        for i in 1..=5 {
            ...
            println!("{}: {:}", i, num);
            assert!(super::super::validate(&num))
        }
