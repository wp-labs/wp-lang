...
pub fn fetch_wpl_data(path: &str, target: &str) -> WplCodeResult<Vec<WplCode>> {
    ...
    for f_name in &files {
        ...
        let mut f = File::open(f_name).owe(WplCodeReason::from_conf()).with_context(&ctx)?;
        let mut buffer = Vec::with_capacity(10240);
        f.read_to_end(&mut buffer).owe(WplCodeReason::from_conf()).with_context(&ctx)?;
        let file_data = String::from_utf8(buffer).owe(WplCodeReason::from_conf()).with_context(&ctx)?;
...
fn find_conf_files(path: &str, target: &str) -> WplCodeResult<Vec<PathBuf>> {
    ...
    for entry in glob(glob_path.as_str()).map_err(|e| {
        WplCodeError::from(WplCodeReason::Syntax(format!("read_dir fail: {}, {}", path, e)))
    })? {
        match entry {
            Ok(path) => {
                found.push(path);
