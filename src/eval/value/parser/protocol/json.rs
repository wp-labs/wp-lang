...
mod tests {
    ...
    #[test]
    fn test_json_std() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_base64() -> WplCodeResult<()> {
        let conf = wpl_rule
    ...
    #[test]
    fn test_json_long() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_unknown_json_fixture_fails_when_sample_is_not_valid_json() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_preserves_garbled_unicode_when_json_is_valid() -> WplCodeResult<()> {
        let rule =
    ...
    #[test]
    fn test_json_works_without_explicit_json_like_pipe() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_rejects_plain_text_without_explicit_json_like_pipe() -> WplCodeResult<()> {
        let rule = r#"rule test { (json(chars@host, chars@method)) }"#;
        let pipe = WplEvaluator::from_code(rule)?;
    ...
    #[test]
    fn test_json_still_rejects_broken_json_like_input() -> WplCodeResult<()> {
        let rule = r#"rule test { (json(chars@host, chars@method)) }"#;
        let pipe = WplEvaluator::from_code(rule)?;
    ...
    #[test]
    fn test_json_rename() -> WplCodeResult<()> {
        //let mut data = r#"{"_origin": {"rsp_status": 200, "sip": "10.180.17.50", "public_date": "2023-02-08 17:11:04", "vuln_name": "疑似访问/下载脚本文件", "detail_info": "发现疑似脚本文件下载行为，该类文件可直接在操作系统上执行，可能存在风险。", "solution": "确认该文件访问/下载行为是否为合规行为；如下载成功，检查所下载文件是否为恶意文件；如是，及时删除该文件并检查其是否被执行，杀死异常进程。", "uri": "/download/distribution/1_20230515/6461b42f12f17.bat", "xff": "", "vuln_harm": "该类文件可直接在操作系统上执行，若下载的是恶意脚本，可能存在较高风险。", "vuln_type": "文件下载", "vuln_desc": "发现疑似脚本文件下载行为，该类文件可直接在操作系统上执行，可能存在风险。", "write_date": 1684127059, "dport": 80, "code_language": "", "sport": 62551, "dip": "10.111.48.17", "site_app": ""}, "alarm_sample": 1, "alarm_sip": "10.180.17.50", "alarm_source": 1, "attack_chain": "0x02050000", "attack_org": "", "attack_sip": "10.111.48.17", "attack_type": "文件下载", "branch_id": "QtAVdJgqi", "file_name": "6461b42f12f17.bat", "first_access_time": "2023-05-15T13:04:19.000+0800", "hazard_level": 4, "hazard_rating": "中危", "host_state": "攻击成功", "ioc": "268572087-疑似访问/下载脚本文件", "is_delete": 0, "is_web_attack": "1", "is_white": 0, "nid": "", "repeat_count": 1, "rule_desc": "网页漏洞利用", "rule_key": "webids", "rule_state": "green", "sip_ioc_dip": "7873ecac0a91ce2b9f96b9f955e82065", "skyeye_id": "", "skyeye_index": "", "skyeye_type": "webids-webattack_dolog", "super_attack_chain": "0x02000000", "super_type": "攻击利用", "type": "文件下载", "type_chain": "16120000", "update_time": 1684127441, "vuln_type": "疑似访问/下载脚本文件", "white_id": null, "x_forwarded_for": "", "rule_labels": "{\"0x110A02\": {\"parent_name\": \"资产识别\", \"name\": \"UA指纹识别\", \"os\": \"Windows/XP\", \"parent_id\": \"0x110A00\", \"role\": \"C\", \"software\": \"IE/6.0\", \"type\": \"模块名称\", \"hw_type\": \"PC\"}}", "att_ck": "初始访问:TA0001|利用面向公众的应用程序:T1190", "serial_num": "QbJK/tb6A", "access_time": "2023-05-15T13:04:19.000+0800", "file_md5": "64f9bfa67c0b89fdb82b0673c1a96964", "host": "10.111.48.17", "rule_id": "0x100215b7", "rsp_status": "200", "skyeye_serial_num": "QbJK/tb6A", "host_md5": "6470cf2c183004f8ead3e150277b4d01", "alarm_id": "20230515_d3b0cca9ea8e9cca348f08ee036103dc", "payload": {"req_header": "", "rsp_header": "", "rsp_body": "", "req_body": ""}, "proto": "http", "sip": "10.180.17.50", "dip": "10.111.48.17", "src_mac": "00:15:c7:f5:90:00", "dst_mac": "00:21:a0:0c:cc:40", "vlan_id": "", "sport": 62551, "dport": 80, "asset_group": "未分配资产组", "dip_group": "未分配资产组", "sip_group": "未分配资产组", "sip_addr": "局域网", "dip_addr": "局域网", "confidence": "高"}"#;
        let mut data =
    ...
    #[test]
    fn test_json_tianyan() -> WplCodeResult<()> {
        let conf = wpl_rule
    ...
    #[test]
    fn test_json_tianyan3() -> WplCodeResult<()> {
        let conf = wpl_rule
            .parse(
    ...
    #[test]
    fn test_json_jt1() -> WplCodeResult<()> {
        let conf = wpl_rule
    ...
    #[test]
    fn test_json_long_keys_no_panic() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_long_parent_with_array_no_panic() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_pipe_auto_last_behavior() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_big_integer_downgrade_to_string() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_i64_and_float_preserve() -> WplCodeResult<()> {
        let conf = WplField::try_parse("json").assert();
    ...
    #[test]
    fn test_json_8_1() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_8_2_0() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_8_2_1() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_8_3() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_8_3_1() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_8_4() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_8_5() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_bug1() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_take_chars() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_take_chars2() -> WplCodeResult<()> {
    ...
    #[test]
    fn test_json_symbol() -> WplCodeResult<()> {
